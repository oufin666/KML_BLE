...
typedef struct 
{
    ...
    uint32_t ic_type;                               // 芯片类型，0：gh3x2x，1：gh3300
}goodix_ecg_config;
...
typedef struct
{
    ...
    int32_t pace;                                   //pace数据
    uint8_t pace_en_flag;                           //pace有效标志位
    uint8_t data_type;                              //数据类型，0：ADC，1：电压
...
* @return     函数执行情况
*/
DRVDLL_API goodix_ecg_ret goodix_ecg_version(uint8_t version[150]);
