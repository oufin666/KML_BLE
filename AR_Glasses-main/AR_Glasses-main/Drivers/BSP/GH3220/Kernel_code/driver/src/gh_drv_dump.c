...
/// dump mode
GU16 g_usDumpMode;   //bit0: GH3X2X_DUMP_GAIN_CURRENT_ENABLE_BIT  bit1: GH3X2X_DUMP_AMBIANCE_ENABLE_BIT  bit2: GH3X2X_DUMP_ELECTR_WEAR_ENABLE_BIT

/// electrode wear status
GU8 g_uchElectrodeWearStatus;

/// save bg cancel value of every channel
extern GU8  g_puchGainBgCancelRecord[GH3X2X_CHANNEL_MAP_MAX_CH];
/// save agc info of every channel
// extern STGh3x2xAgcInfo g_stGh3x2xAgcInfo;
/**
...
const STGh3x2xAgcReg g_pstGH3x2xAgcReg[] = 
{
    ...
    {0x16, 7,0},               //16: rx0 dc cancel
    {0x16, 15,8},              //17: rx1 dc cancel
    {0x18, 7,0},               //18: rx2 dc cancel
    {0x18, 15,8},              //19: rx3 dc cancel
};
        ...
        (GH3X2X_CHECK_LEFT_BIT_NOT_SET(g_usDumpMode, GH3X2X_DUMP_AMBIANCE_ENABLE_BIT)))
    {
        ...
        for (uchSlotIndexCnt = 0; uchSlotIndexCnt < GH3X2X_MAX_SLOT_NUM; uchSlotIndexCnt++)
        {
            ...
            usDrv1CurRegData = (GH3X2X_ReadReg(usDrv1CurRegAddr + (uchSlotIndexCnt * GH3X2X_SLOT_CTRL_OFFSET)) & GH3X2X_DRV_CURRENT_BIT_MASK);
						g_pstGh3x2xAgcInfo->uchDrvCurrentRecord[uchSlotIndexCnt] =  (GU16)(usDrv0CurRegData << GH3X2X_DRV0_CURRENT_LEFT_SHIFT_BIT_NUM);
            g_pstGh3x2xAgcInfo->uchDrvCurrentRecord[uchSlotIndexCnt] |= (GU16)(usDrv1CurRegData << GH3X2X_DRV1_CURRENT_LEFT_SHIFT_BIT_NUM);
        }
        GH3X2X_WAIT_CHIP_DSLP();
    }
}
 ...
 *
 * @brief  record tia gain and drv information
 *
 * @attention   None
 *
 * @param[in]   None
...
{
    GH3X2X_Memcpy(g_pstGh3x2xAgcInfo->uchTiaGainRecord, g_pstGh3x2xAgcInfo->uchTiaGainAfterSoftAgc, sizeof(g_pstGh3x2xAgcInfo->uchTiaGainRecord));
    GH3X2X_Memcpy(g_pstGh3x2xAgcInfo->uchDrvCurrentRecord, g_pstGh3x2xAgcInfo->uchDrvCurrentAfterSoftAgc, sizeof(g_pstGh3x2xAgcInfo->uchDrvCurrentRecord));
    GH3X2X_Memcpy(g_pstGh3x2xAgcInfo->uchDcCancelRecord, g_pstGh3x2xAgcInfo->uchDcCancelAfterSoftAgc, sizeof(g_pstGh3x2xAgcInfo->uchDcCancelRecord));
    GH3X2X_INFO_LOG_PARAM_AGC("[%s] ----------------------------------------------------\r\n", __FUNCTION__);
    GH3X2X_INFO_LOG_PARAM_AGC("[%s]GAIN      slot0: adc0:%d, adc1:%d,adc2:%d,adc3:%d\r\n", __FUNCTION__,g_pstGh3x2xAgcInfo->uchTiaGainRecord[0],g_pstGh3x2xAgcInfo->uchTiaGainRecord[1],g_pstGh3x2xAgcInfo->uchTiaGainRecord[2],g_pstGh3x2xAgcInfo->uchTiaGainRecord[3]);
    GH3X2X_INFO_LOG_PARAM_AGC("[%s]DRV       slot0: adc0:%d, adc1:%d,adc2:%d,adc3:%d\r\n", __FUNCTION__,g_pstGh3x2xAgcInfo->uchDrvCurrentRecord[0],g_pstGh3x2xAgcInfo->uchDrvCurrentRecord[1],g_pstGh3x2xAgcInfo->uchDrvCurrentRecord[2],g_pstGh3x2xAgcInfo->uchDrvCurrentRecord[3]);
    GH3X2X_INFO_LOG_PARAM_AGC("[%s]DC Cancel slot0: adc0:%d, adc1:%d,adc2:%d,adc3:%d\r\n", __FUNCTION__,g_pstGh3x2xAgcInfo->uchDcCancelRecord[0],g_pstGh3x2xAgcInfo->uchDcCancelRecord[1],g_pstGh3x2xAgcInfo->uchDcCancelRecord[2],g_pstGh3x2xAgcInfo->uchDcCancelRecord[3]);
    GH3X2X_INFO_LOG_PARAM_AGC("[%s] ----------------------------------------------------\r\n", __FUNCTION__);
}
...
GS8 GH3X2X_FindLastSlotChMapIndex(GU8* puchTargetChMapIndex, GU8 puchChMapArr[], GU8 uchChMapCnt, GU8 uchChCntIndex)
{
    ...
    if (i >= uchChMapCnt)
    {
        *puchTargetChMapIndex = uchChCntIndex;
        GH3X2X_DEBUG_LOG_PARAM("Find channel map error!!!can not find last slot channel map\r\n");
                                ...
                                GU16 usReadGsensorArrCnt)
{
    ...
    for (uchChCntIndexTmp = 0; uchChCntIndexTmp < uchChannelCnt; uchChCntIndexTmp++)
    {
        ...
        else
        {
            ...
            {
                GH3X2X_DEBUG_LOG_PARAM("dump data of buffer error!\r\n");
                break;
            }
        }
    ...
    if (uchDumpDataCntCalcErrFlag == 0)
    {
        ...
        else
        {
            ...
            {
                GH3X2X_DEBUG_LOG_PARAM("dump data doesn't correspond to channel map error!\r\n");
                usRawdataCntTmp = 0;
                    ...
                    GU8 *puchChannelBgLvlArr)
{
    ...
    for (uchChCntIndex = 0; uchChCntIndex < uchChannelMapCnt; uchChCntIndex++)
    {
        ...
        {
            GH3X2X_DEBUG_LOG_PARAM("dump lvl error! set @ref EMGh3x2xDumpRawdataBgLvlSelect!\r\n");
            return GH3X2X_RET_PARAMETER_ERROR;
        }
    }
                                            ...
                                            GU16 usGsDataNum, EMGsensorSensitivity emGsSensitivity)
{
    ...
    else
    {
        ...
        GH3X2X_DEBUG_LOG_PARAM("hba hasn't started error!\r\n");
    }
    return chRet;
}
                                            ...
                                            GU16 usGsDataNum, EMGsensorSensitivity emGsSensitivity)
{
    ...
    {
        chRet = GH3X2X_RET_GENERIC_ERROR;
        GH3X2X_DEBUG_LOG_PARAM("hrv hasn't started error!\r\n");
    }
    return chRet;
}
                                ...
                                GU16 usGsDataNum, EMGsensorSensitivity emGsSensitivity)
{
    ...
    {
        chRet = GH3X2X_RET_GENERIC_ERROR;
        GH3X2X_DEBUG_LOG_PARAM("spo2 hasn't started error!\r\n");
    }
    return chRet;
}
    ...
    for (uchChIndex = 0; uchChIndex < GH3X2X_CHANNEL_MAP_MAX_CH; ++uchChIndex)
    {
        ...
        for (uchDumpDataCnt = 0; uchDumpDataCnt < uchDumpDataNum; ++uchDumpDataCnt)
        {
            ...
            if (GH3X2X_CHECK_LEFT_BIT_SET(punLastIncompleteFlagArr[uchChIndex], uchDumpDataCnt))
            {
                ...
                {
                    GH3X2X_DEBUG_LOG_PARAM("Cache incomplete data too much!\r\n");
                }
            }
            else
            {
