    ...
    */
    ...
    while(uchNeedContinue)
    {
        ...
        if(GH3X2X_UPLOAD_STATUS_HAVE_STORE_DATA == g_uchGh3x2xUploadStatus && uchFillLastFrameDataFlag == 0)
        {
            ...
            {
                puchTempFrameDataBuf[uchOneFrameByteNum] = 0;
                uchOneFrameByteNum ++;  
                #if __SUPPORT_SOFT_AGC_CONFIG__  
                pstFrameInfo->punFrameFlag[0] = g_NewAgcMainChelFlag;
                #endif
                GH3X2X_Memcpy((GU8*)&(puchTempFrameDataBuf[uchOneFrameByteNum]), (GU8*)(&(pstFrameInfo->punFrameFlag[0])), 4);
                uchOneFrameByteNum += 4;
                #if __SUPPORT_SOFT_AGC_CONFIG__
                g_NewAgcMainChelFlag = 0;
                #endif
            }
            else
            {
            ...
            //flag2
            if ((*(pstFrameInfo->punFrameCnt)) == 0 || pstFrameInfo->punFrameFlag[2] != g_unLastFrameFlag2 || uchFlag2ChangeFlag)
            {
                ...
                uchFlag2ChangeFlag = 1;
            }
            else
            {
                uchFlag2ChangeFlag = 0;
                uchRealReslutByteNum -= 5;
            }
            //flag3
            if(uchFramePosi&GH3X2X_FRAME_POSI_HEAD)
            ...
            #if GH3X2X_DIVIDE_ZERO_FLAG_UPLODE_EN
            // flag4 div 0 flag
            puchTempFrameDataBuf[uchOneFrameByteNum] = 4;
            uchOneFrameByteNum ++;    
            GH3X2X_Memcpy((GU8*)&(puchTempFrameDataBuf[uchOneFrameByteNum]), (GU8*)(&(pstFrameInfo->punFrameFlag[4])), 4);
            uchOneFrameByteNum += 4;    
            #endif
