...
3. **配置Channel2为PWM模式**
   - 点击 `Channel2` 下拉菜单，选择 `PWM Generation CH2`

4. **配置PWM参数**
   - 点击上方 `Parameter Settings` 标签
   - **Clock Source**: Internal Clock
   - **Counter Settings**:
     - `Prescaler (PSC)`: 95（时钟频率96MHz，96MHz/(95+1)=1MHz）
     - `Counter Period (ARR)`: 1000（1MHz/1000=1kHz PWM频率）
   - **Channel 2**:
     - `Mode`: PWM Mode 1
     - `Pulse`: 0（初始占空比0）
     - `Fast Mode`: Disabled
