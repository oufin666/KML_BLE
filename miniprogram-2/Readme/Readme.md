# BLE KML/GPX 发送小程序
通过蓝牙低功耗(BLE)发送KML和GPX文件的微信小程序，支持文件压缩、解压校对等功能。

## 项目概览
本项目提供了一套完整的KML和GPX文件通过BLE传输到硬件设备的解决方案，包括文件选择、压缩、分包发送、设备管理和数据校对等功能。

## 整体流程

### 文件发送流程
1. **文件选择**：用户选择本地KML或GPX文件
2. **文件解析**：根据文件类型自动选择解析器，提取坐标数据
...
### 解压校对流程
1. **文件选择**：用户选择Bin文件和对应的KML或GPX文件
2. **Bin解压**：解压Bin文件得到坐标数据
3. **文件解析**：解析原始文件得到原始坐标数据
4. **数据校对**：比较解压后的数据与原始数据
5. **结果展示**：显示校对结果，包括差异和CRC32校验值

## 功能
- **文件发送**：支持选择、压缩、分包发送KML和GPX文件，实时显示进度
- **BLE设备管理**：扫描、连接、自动重连
- **串口助手**：文本/十六进制模式，收发数据
- **Bin文件解压校对**：解压Bin文件并与KML或GPX文件校对
...
## 技术
- **压缩算法**：针对地理坐标数据优化的差分压缩算法，支持KML和GPX格式
...
3. **文件处理模块** (`utils/kml-sender.ts`)：负责KML和GPX文件解析、数据压缩和发送逻辑
4. **压缩测试模块** (`utils/compression-test.ts`)：用于测试压缩算法性能和准确性

### 函数调用关系
```
主页面
  ├── 选择文件 → kml-sender.ts:readFileAsArrayBuffer()
  ├── 扫描设备 → ble.ts:startScan()
  ├── 连接设备 → ble.ts:connectDevice()
  ├── 发送文件 → kml-sender.ts:sendKmlBufferToBle()
  │   ├── 压缩数据 → kml-sender.ts:compressTextByType()
  ...
  └── 解压校对 → compression-test.ts:decompressAndVerify()
      ...
      ├── 解析文件 → kml-sender.ts:convertFileToBin()
...
### 文件处理模块 (`kml-sender.ts`)
- `readFileAsArrayBuffer(file, compress, fileType)`：读取文件内容并根据类型选择解析器
- `convertFileToBin(text, fileType)`：将文件内容转换为二进制格式
- `compressTextByType(text, fileType)`：根据文件类型压缩数据
- `sendKmlBufferToBle(buffer, options)`：发送数据到BLE设备
